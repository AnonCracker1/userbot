...
async def _(event):
    ...
    #packname = f"AnonymousKaPack3"
    #packshortname = f"AnonymousKaPack3"  # format: Uni_Borg_userid
    if userid == 997495932:
        packname = f"AnonymousKaPack3"
        packshortname = "AnonymousKaPack3"
    else:
        packname = f"{user.first_name}'s Userbot Vol.{pack}"
        packshortname = f"Userbot_{userid}_Pack"
    await event.edit("`Is It Legal ? Oh Yes it is !! \nLook That Way ! Let me Kang This ¯\_(ツ)_/¯`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@anon_cracker.png"
    ...
    if is_a_s:
        ...
        if userid == 997495932:
            packname = f"Anonymous Animated3"
            packshortname = "Anonymous_Animated3"
        else:
            packname = f"{user.first_name}'s Userbot Animated Vol.{pack}"
            packshortname = f"Userbot_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    pack += 1
                    prevv = int(pack) - 1
                    packname = f"AnonymousKaPack4"
                    packshortname = f"AnonymousKaPack4"
                    #if userid == 748471982:
                       # packname = f"AnonymousKaPack4""
                       # packshortname = "AnonymousKaPack4""
                   # else:
                       # packname = f"Userbotttt PACKKKKKKKKKK"
                        #packshortname = f"Vol._{pack}_Userbot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
